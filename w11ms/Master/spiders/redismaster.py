...
class MasterSpider(CrawlSpider):
    ...
    # 进入某一标签后，页面中的详情链接及分页
    links2 = LxmlLinkExtractor(restrict_css=('.paginator .next a',))
    # 进入最终详情页面
    links3 = LxmlLinkExtractor(restrict_css=('.subject-list .subject-item .pic .nbg a',))
    rules = [
        Rule(links1,callback='parse_directory', follow=True),
        Rule(links2,callback='parse_directory', follow=True),
        Rule(links3,callback='parse_directory', follow=True),
    ]
